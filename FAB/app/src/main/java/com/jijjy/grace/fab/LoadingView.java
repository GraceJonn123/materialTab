...
import com.jijjy.grace.fab.model.Circle;
...
/*
* Callback listener for animations while they're staring/ending/repeating
*/
    ...
    private int mDuration = 0; //default iz 500 milliseconds..
    private int mRepeats = 1; //default iz one(1)

    //stores all loading items
    private SparseArray<io.saeid.fabloading.LoadingView.LoadingItem> mItems = new SparseArray<>();
    ...
    private io.saeid.fabloading.LoadingView.LoadingItem mCurrentLoadingItem;
    //Current running circle
    private Circle mCircle = new Circle();
    //The value of position change in each frame
    private float mStep;
    //A path for floating action bar
    ...
    //offset btw fab and the real layout bonds(non-zero in pre lollipop)
    ...
    public LoadingView addAnimation(int color, int drawable, @AnimationMode int type) {
        if (!mRunning) {
            ...
            mItems.put(mItems.size(), new io.saeid.fabloading.LoadingView.LoadingItem(color, drawable, type));
        }
        return this;
    }
